...
fn load_texture(filename: []const u8, texture: *arrayList(Color), text_size: *usize, text_cnt: *usize) !void {
    zstbi.init(std.heap.page_allocator);
    defer zstbi.deinit();

    const pixmap = try zstbi.loadFromFile(filename, 0);
    // defer pixmap.deinit();

    if (!pixmap) return error.TextureLoadError;
    if (pixmap.num_components != 4) return error.TextureNumChannelsError;
    if (pixmap.width != pixmap.height) return error.TextureSquareError;

    text_cnt = pixmap.width / pixmap.height;
    text_size = pixmap.width / text_cnt;

    var x: usize = 0;
    while (x < (pixmap.width * pixmap.height)) : (x += 1) {
        var r = pixmap.data[(x * 4) + 0];
        var g = pixmap.data[(x * 4) + 1];
        var b = pixmap.data[(x * 4) + 2];
        var a = pixmap.data[(x * 4) + 3];
